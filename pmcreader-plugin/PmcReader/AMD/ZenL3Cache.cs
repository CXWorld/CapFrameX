...
{
    public class ZenL3Cache : Amd17hCpu
    {
        ...
        public ZenL3Cache()
        {
            ...
            {
                int rawCcxIdx = GetCcxId(threadIdx);
        ...
        {
            private ZenL3Cache l3Cache;

            public HitRateLatencyConfig(ZenL3Cache l3Cache)
            {
            ...
            {
                ulong L3AccessPerfCtl = GetL3PerfCtlValue(0x01, 1 << 7, true, 0xF, 0xFF); // bit 7 = caching: L3 cache access
                ulong L3MissPerfCtl = GetL3PerfCtlValue(0x06, 0x01, true, 0xF, 0xFF); // bit 0 = requestmiss
                ...
                ulong L3ReqLookupState = GetL3PerfCtlValue(0x04, 0xFF, true, 0xF, 0xFF);
                ulong L3ReqMiss = GetL3PerfCtlValue(0x04, 0x1, true, 0xF, 0xFF);
                ...
                foreach(KeyValuePair<int, int> ccxThread in l3Cache.ccxSampleThreads)
                {
                    ...
                    Ring0.WriteMsr(MSR_L3_PERF_CTL_4, L3ReqLookupState);
                    Ring0.WriteMsr(MSR_L3_PERF_CTL_5, L3ReqMiss);
                }
            }
