...
- `msbuild source\CapFrameXInstaller\CapFrameXInstaller.wixproj /p:Configuration=Release /p:Platform=x64` builds the WiX installer. It uses WiX Toolset v6.0.2 as the `WixToolset.Sdk` MSBuild SDK restored from NuGet (add `/restore`), so nothing has to be installed. Its file list is harvested from the app output at build time by the `<Files>` element in `Product.wxs`; build the app first, and add exclusions there as `<Exclude>` entries.
- `vstest.console source\CapFrameX.Test\bin\x64\Release\net10.0-windows\CapFrameX.Test.dll /Platform:x64` runs unit tests after building the test project. Mind the target-framework folder — a stale `bin\x64\Release\CapFrameX.Test.dll` from the .NET Framework era may still be lying around and would run silently.

See `.github/workflows/main.yml` for the CI build ordering and Visual Studio version settings.
