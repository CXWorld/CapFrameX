...
This branch develops **CapFrameX 2.0**: the shipping 1.9.x application in `source/`, and next to it the rewrite with a shared service core, one service per platform, and an Angular frontend. Plans and decisions are in `dev-plans/`.

# Repository Structure

| Folder | Contents |
| --- | --- |
| `source/` | The 1.9.x WPF application and the libraries both generations use |
| `CapFrameX.Service.Shared/` | Platform-neutral service core: contracts, platform ports, application logic, records, analysis, data, and the HTTP/SSE/WebSocket API as a library |
| `CapFrameX.Service.Windows/` | Windows service: PresentMon capture, PawnIO-backed telemetry, hotkeys, overlay, installer |
| `CapFrameX.Service.Linux/` | Linux service: Vulkan layer capture, kernel telemetry, XDG paths |
| `CapFrameX.UI/` | Angular frontend and its desktop shell |
| `dev-plans/` | Development plans and the architecture diagram |

Both services expose the same API, so one frontend serves either platform. Build a service with its own solution:

```bash
dotnet build CapFrameX.Service.Windows/CapFrameX.Service.Windows.sln
dotnet build CapFrameX.Service.Linux/CapFrameX.Service.Linux.sln
```
