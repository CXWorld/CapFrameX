...
namespace CapFrameX.Test.Updater
{
	...
	{
		private static readonly Version InstalledVersion = new Version(1, 9, 0);
		...
		{
			var leftoverPackage = WritePackage("CapFrameX-1.9.1-setup.exe");
		...
		{
			var package = WritePackage("CapFrameX-1.9.1-setup.exe");
		...
		{
			var package = WritePackage("CapFrameX-1.9.0-setup.exe");
			StageMarker("1.9.0", Path.GetFileName(package), PackageSha256(package));
		...
		{
			var package = WritePackage("CapFrameX-1.9.1-setup.bat");
			StageMarker("1.9.1", Path.GetFileName(package), PackageSha256(package));
		...
		{
			var package = WritePackage("CapFrameX-1.9.1-setup.exe");
			StageMarker("1.9.1", Path.GetFileName(package), new string('a', 64));
		...
		public void TryStartPendingUpdate_PackagePathOutsideUpdatesFolder_DiscardsStagedUpdate()
		{
			...
			var package = WritePackage("CapFrameX-1.9.1-setup.exe");
			StageMarker("1.9.1", @"..\..\Windows\System32\cmd.exe", PackageSha256(package));
